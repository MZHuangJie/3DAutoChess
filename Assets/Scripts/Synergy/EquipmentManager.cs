...
namespace AutoChess.Core
{
    public class EquipmentManager : MonoBehaviour
    {
        ...
        {
            if (piece == null || item == null) return false;

            if (item.equipmentType == EquipmentType.Base)
            {
                var partner = FindBasePartner(piece, item);
                if (partner != null)
                {
                    var combined = FindCombineResult(item, partner);
                    piece.RemoveEquipment(partner);
                    owner.equipmentInventory.Remove(item);
                    piece.Equip(combined);
                    Debug.Log($"[Equipment] Combined {item.equipmentName} + {partner.equipmentName} = {combined.equipmentName}");
                    return true;
                }
            }

            if (!piece.CanEquip) return false;
        ...
        public void TryCombineEquipment(ChessPiece piece)
        {
            TryCombine(piece, null);
        }
        ...
        EquipmentData FindBasePartner(ChessPiece piece, EquipmentData newBase)
        {
            foreach (var existing in piece.equipment)
            {
                if (existing == null || existing.equipmentType != EquipmentType.Base) continue;
                if (FindCombineResult(newBase, existing) != null)
                    return existing;
            }
            return null;
        }
